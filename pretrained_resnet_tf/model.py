...
def model(x_train, y_train, x_test, y_test):
    """
    Fine-tune for mnist a Resnet50v2 model trained on ImageNet dataset
    ...
    model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 0.01), loss = tf.keras.losses.CategoricalCrossentropy(from_logits=True), metrics=['accuracy'])
    ...
    model.fit(x_train_preproc, y_train, epochs = 5, batch_size = 128*2)